<?php
	...
	$sql = "SELECT manifestacao.id, manifestacao.usuario_id, manifestacao.tipo_id, manifestacao.setor_id, manifestacao.relevancia_id, manifestacao.status_id, assunto, corpo, data, anexo, tipo, setor, relevancia, status, nome, matricula, email FROM manifestacao LEFT OUTER JOIN usuario ON manifestacao.usuario_id = usuario.id LEFT OUTER JOIN setor ON manifestacao.setor_id = setor.setor_id LEFT OUTER JOIN tipo ON manifestacao.tipo_id = tipo.tipo_id LEFT OUTER JOIN status ON manifestacao.status_id = status.status_id LEFT OUTER JOIN relevancia ON manifestacao.relevancia_id = relevancia.relevancia_id";

	$stmt = $elo->prepare($sql); 
	$stmt->execute();

	$manifestos = $stmt->fetchAll(PDO::FETCH_ASSOC);

	for ($y=0; $y < count($manifestos); $y++) { 
		if ($id == $manifestos[$y]['id']) {
			$manifesto = $manifestos[$y];
		}
	}

	$datetime = explode(" ", $manifesto['data']);
	$date = explode("-", $datetime[0]);
	$date = $date[2]."/".$date[1]."/".$date[0];

	$manifesto['data'] = $date." às ".$datetime[1];


	$sql = "SELECT * FROM resposta WHERE manifestacao_id = ?";

	$stmt = $elo->prepare($sql); 
		$stmt->bindParam(1, $manifesto['id']);
	$stmt->execute();

	$respostas = $stmt->fetchAll(PDO::FETCH_ASSOC);

	for ($y=0; $y < count($respostas); $y++) {
		$datetime = explode(" ", $respostas[$y]['data']);
		$date = explode("-", $datetime[0]);
		$date = $date[2]."/".$date[1]."/".$date[0];

		$respostas[$y]['data'] = $date." às ".$datetime[1];
	}
?>
...
<head>
	...
	<title>Manifestação | OuvEAJ</title>
...
<body>
	<?php if(logado()): ?>
		<?php
			switch ($manifesto['tipo_id']) {
			    case 1: $_SESSION['tipo'] = 1; $_SESSION['cor'] = "verde"; $_SESSION['img'] = "likea.svg"; $_SESSION{'def'} = "Elogio";
			        break;
			   case 2: $_SESSION['tipo'] = 2; $_SESSION['cor'] = "laranja"; $_SESSION['img'] = "unlikea.svg"; $_SESSION{'def'} = "Reclamação";
			        break;
			   case 3: $_SESSION['tipo'] = 3; $_SESSION['cor'] = "vermelho"; $_SESSION['img'] = "altofalantea.svg"; $_SESSION{'def'} = "Denúncia";
			        break;
			   case 4: $_SESSION['tipo'] = 4; $_SESSION['cor'] = "azulmc"; $_SESSION['img'] = "lampadaa.svg"; $_SESSION{'def'} = "Sugestão";
			        break;
			   case 5: $_SESSION['tipo'] = 5; $_SESSION['cor'] = "roxo"; $_SESSION['img'] = "papela.svg"; $_SESSION{'def'} = "Solicitação";
			        break;
			...
			<div>
				<section class="row">
						...
						<div class="container main col-9 col row branco rad-1">
							<div>
								<div class="row col-12 col">
									<h2 class="f20 bottom">Manifestação</h2>
								</div>
								<hr class="hr azule">
							</div>
							<div>
								<div class="row col-12">	
									<div class="row col col-5">
										<div class="col col-12 row grups">
											<p class="bottom">Assunto: <strong><?php echo $manifesto['assunto']; ?></strong></p>
											<hr>
										</div>
										<div class="col col-12 row grups">
											<p>Status: <strong><?php echo $manifesto['status']; ?></strong></p>
										</div>
										<div class="col col-12 row grups">
											<div class="col col-6">
												<p>Setor: <strong><?php echo $manifesto['setor']; ?></strong></p>
											</div>
											<div class="col col-6">
												<p>| Relevância: <strong><?php echo $manifesto['relevancia']; ?></strong></p>
											</div>
										</div>
										<div class="col col-12 row grups">
											<?php if($manifesto['anexo'] != null): ?>
												<a class="fbranco" href="php/baixar.php?anexo=<?php echo $manifesto['anexo']; ?>">
													<div class="hideinp row col col-12 azule hover rad-5 fbranco center upcase f15">
														<span><span class="icon-attachment"></span> Vizualizar anexo</span>
													</div>
												</a>
											<?php else: ?>
												<div class="hideinp row col col-12 azule rad-5 fbranco center upcase f15 right">
													<span><span class="icon-attachment"></span> Nenhum anexo submetido</span>
												</div>
											<?php endif; ?>
										</div>
										<div class="col col-12 row grups center">
											<p><span class='icon-clock'></span><strong> <?php echo $manifesto['data']; ?></strong></p>
										</div>
										<?php if(((ouvidor()) and ($_SESSION['ouvidor_id'] == $manifesto['setor_id'])) or ($manifesto['usuario_id'] == $_SESSION['id'])): ?>
											<div class="col col-12 row">
												<div class="col col-12 row bottom">
													<br>
													<p class="bottom cinzac f12"><span class="icon-info"></span> As seguintes informações são visivéis apenas ao autor da manifestação e ao ouvidor responsável pelo setor alvo</p>
												</div>
												<div class="col col-12 row bottom">
													<p class="bottom">Manifestante: <strong><?php echo $manifesto['nome']; ?></strong></p>
												</div>
												<div class="col col-12 row bottom">
													<p class="bottom">Matrícula: <strong><?php echo $manifesto['matricula']; ?></strong></p>
												</div>
												<div class="col col-12 row bottom">
													<p class="bottom">E-mail: <strong><?php echo $manifesto['email']; ?></strong></p>
												</div>
											</div>
										<?php endif; ?>								
									</div>										
									<div class="row col col-7">
										<div class="col col-12 grups on">
											<textarea class="inp col col-12 text-area" type="text" name="corpo" disabled><?php echo $manifesto['corpo']; ?></textarea>
										</div>
										<div class="col col-12 grups off">
											<?php if(count($respostas) <= 0): ?>
												<div class="f20 center col col-12 text-area">
													<p class="top">Nenhuma resposta submetida</p>
												</div>
												
											<?php else: ?>
												<ul class="scroll text-area cinzac">
													<?php for ($y=(count($respostas)-1); $y >= 0 ; $y--) {
														echo "<li class='row col-12 li'>
																<div class='row col-10 skp-1 f12 fazul bottom'>
																	<p><span class='icon-clock'></span> ".$respostas[$y]['data']."</p>
																</div>
																<div class='row col-10 skp-1'>
																	<p>".$respostas[$y]['resposta']."</p>
																</div>
															</li>";
													} ?>
												</ul>
										...
										<div class="col-12 row">
											<?php  if((ouvidor()) and ($_SESSION['ouvidor_id'] == $manifesto['setor_id'])): ?>
												<div class="col col-6">
													<button onclick="resposta(event);" class="btn hover col col-12 verde fbranco upcase rad-1"><span class="icon-pencil"></span> Responder Manifestação</button>
												</div>	
											<?php endif; ?>
											<div class="col col-6 right on">
												<button onclick="x(1);" class="btn hover col col-12 azule fbranco upcase rad-1">Vizualizar respostas</button>
											</div>
											<div class="off col col-6 right">
												<button onclick="x(2);" class="btn hover col col-12 azule fbranco upcase rad-1">Vizualizar manifestação</button>
											</div>	
										</div>
									</div>
								</div>
					...
					<div class="modal azuloe row col-12">
						...
						<?php  if((ouvidor()) and ($_SESSION['ouvidor_id'] == $manifesto['setor_id'])): ?>
							<div class="resposta col-6 col row azulme fbranco">
								<div class="row col-12">	
									<form action="php/responder.php" method="post">
										<div class="row col col-7 skp-4">
											<br><br><br>
											...
											<div class="col-12 row grups">
												<label class="col-4 col">Status</label>
												<select class="select col-8 col" style="background-color: white;" name="status">
											...
											<div class="col-12 grups row">
												<textarea class="inp col col-12 text-area" name="resposta" placeholder="Insira sua resposta..."></textarea>
											</div>
											<div class="col-12 right row">
												<button name="m_id" value="<?php echo $manifesto['id']; ?>" onclick="resposta(event);" class="btn hover col col-12 verde fbranco upcase rad-1">Submeter Resposta</button>
											</div>
										</div>
									</form>
								</div>
