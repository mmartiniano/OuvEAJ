<?php
	...
	if(!selecionado()){
		...
		$sql = "SELECT * FROM manifestacao LEFT OUTER JOIN setor ON manifestacao.setor_id = setor.setor_id LEFT OUTER JOIN tipo ON manifestacao.tipo_id = tipo.tipo_id LEFT OUTER JOIN status ON manifestacao.status_id = status.status_id LEFT OUTER JOIN relevancia ON manifestacao.relevancia_id = relevancia.relevancia_id WHERE assunto LIKE '%".$search."%' OR tipo LIKE '%".$search."%' OR setor LIKE '%".$search."%' OR relevancia LIKE '%".$search."%' OR status LIKE '%".$search."%' OR corpo LIKE '%".$search."%' OR data LIKE '%".$search."%' ;";
		...
		if(isset($tipo)){
			for ( $y = (count($busca)-1); $y >= 0; $y--) {
				if(!((($tipo == $busca[$y]['tipo_id']) or ($tipo == "null")) and (($setor == $busca[$y]['setor_id']) or ($setor == "null")) and (($relevancia == $busca[$y]['relevancia_id']) or ($relevancia == "null")) and (($status == $busca[$y]['status_id']) or ($status == "null")))){
					unset($busca[$y]);
				}
			}
			$busca = array_values($busca);
		}
			...
			<header class="topbar text-right col col-12 circo azule">
				<nav class="row">
					...
					<form action="pesquisa.php" class="col col-4 search-box transparente">
						<input type="search" name="search" class="search-inp inp big-ib col col-12" placeholder="Pesquisar Manifestações..." value="<?php  echo $search;?>">
						...
						<div class="container main col-9 col row branco rad-1">
							<div>
								<div class="row col-12 col">
									<h2 class="f20 bottom">Pesquisa</h2>
								</div>
								<hr class="hr azule">
							</div>
							<div>
								<div class="row bottom">
									<form class="col col-12" action="pesquisa.php" method="get">
										<div class="col col-2 row">
											<p class="col col-4">Tipo</p>
											<select class="select col-8 col" name="tipo">
												<option value=null>-----</option>
												<option <?php if((isset($tipo) and ($tipo == 1))) {echo "selected";}?> value="1">Elogio</option>
												<option <?php if((isset($tipo) and ($tipo == 2))) {echo "selected";}?> value="2">Reclamação</option>
												<option <?php if((isset($tipo) and ($tipo == 3))) {echo "selected";}?> value="3">Denúncia</option>
												<option <?php if((isset($tipo) and ($tipo == 4))) {echo "selected";}?> value="4">Sugestão</option>
												<option <?php if((isset($tipo) and ($tipo == 5))) {echo "selected";}?> value="5">Solicitação</option>
											</select>
										</div>
										<div class="col col-3 row">
											<p class="col col-4">Setor</p>
											<select class="select col-8 col" name="setor">
												<option value=null>-----</option>
												<option <?php if((isset($setor) and ($setor == 1))) {echo "selected";}?> value="1">Agroindústria</option>
												<option <?php if((isset($setor) and ($setor == 2))) {echo "selected";}?> value="2">Agropecuária</option>
												<option <?php if((isset($setor) and ($setor == 3))) {echo "selected";}?> value="3">Aquicultura</option>
												<option <?php if((isset($setor) and ($setor == 4))) {echo "selected";}?> value="4">Ensino Médio</option>
												<option <?php if((isset($setor) and ($setor == 5))) {echo "selected";}?> value="5">Graduação</option>
												<option <?php if((isset($setor) and ($setor == 6))) {echo "selected";}?> value="6">Informática</option>
												<option <?php if((isset($setor) and ($setor == 7))) {echo "selected";}?> value="7">Internato</option>
											</select>
										</div>
										<div class="col col-3">
											...
											<select class="select col-6 col" name="relevancia">
												<option value=null>-----</option>
												<option <?php if((isset($relevancia) and ($relevancia == 1))) {echo "selected";}?> value="1">Alta</option>
												<option <?php if((isset($relevancia) and ($relevancia == 2))) {echo "selected";}?> value="2">Média</option>
												<option <?php if((isset($relevancia) and ($relevancia == 3))) {echo "selected";}?> value="3">Baixa</option>
											</select>
										</div>
										<div class="col col-3">
											...
											<select class="select col-8 col" name="status">
												<option value=null>-----</option>
												<option <?php if((isset($status) and ($status == 1))) {echo "selected";}?> value="1">Resolvido</option>
												<option <?php if((isset($status) and ($status == 2))) {echo "selected";}?> value="2">Em andamento</option>
												<option <?php if((isset($status) and ($status == 3))) {echo "selected";}?> value="3">Enviado</option>
											</select>
										</div>
										<div class="col col-1">
											<button type="submit" name="search" value="<?php echo $_GET['search']; ?>" class="btn hover center col-12 azule fbranco upcase  rad-5">Filtrar</button>
										</div>
									</form>
								</div>
								<div class="row">
									<div class="col col-12 cinzac">
										<?php if(count($busca) <= 0): ?>
											<div class="f20 center col col-12 show">
												<p class="top">Nenhum resultado encontrado</p>
											</div>
										<?php else: ?>
											...
											<ul class="show row col-12 scroll">
												<?php for ($y=(count($busca)-1); $y >= 0 ; $y--) {
													if($busca[$y]['relevancia'] == null){
														$busca[$y]['relevancia'] = "---";
													} 
													echo "<li class='row col-12 li'>
															<a href='manifesto.php?id=".$busca[$y]['id']."' class='l2 fpreto'>
																<div class='col col-4'>
																	<p>".$busca[$y]['assunto']."</p>
																</div>
																<div class='col col-2'>
