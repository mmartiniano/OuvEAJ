<?php
	...
	for ($y=0; $y < count($_SESSION['usuarios']); $y++) { 
		if ($id == $_SESSION['usuarios'][$y]['id']) {
			$usuario = $_SESSION['usuarios'][$y];
		}
	}

	if($usuario['ouvidor_id']){
		$cargo = "Ouvidor";
	}elseif($usuario['administrador_id']){
		$cargo = "Administrador";
	}else{
		$cargo = "Manifestante";
	}
						...
						<div class="container main col-9 col row branco rad-1">
							<div class="grups">
								<div class="row col-12 col">
									<h2 class="f20 bottom">Definir Usuário como Ouvidor</h2>
								</div>
								<hr class="hr azule">
							</div>
							<div>
								<div class="row col-12">	
									<div class="row col col-5">
										<div class="col col-12 row grups">
											<p class="bottom">Nome: <strong><?php echo $usuario['nome']; ?></strong></p>
											<hr>
										</div>
										<div class="col col-12 row grups top">
											<div class="skp-4 circle branco big overhide foto-perfil">
												<?php if (isset($usuario['perfil'])) : ?>
													<img src="<?php  echo $usuario['perfil'];?>" alt="Perfil" class="img col-12">	
												<?php else: ?>
													<img src="img/userg.svg" alt="Perfil" class="img col-12">
												<?php endif; ?>	
											</div>
										...
										<div class="col col-12 row">
											<?php if (! $usuario['ouvidor_id']): ?>
												<a href="php/definir.php?id=<?php echo $usuario['id']."&setor_id=".$usuario['setor_id']; ?>" class="btn center hover col col-12 azule fbranco upcase rad-5"><span class="icon-user-check"></span> Definir usuário como ouvidor</a>
											<?php else: ?>
												<a href="php/definir.php?id=<?php echo $usuario['id']."&setor_id=".$usuario['setor_id']; ?>" class="btn center hover col col-12 vermelho fbranco upcase rad-5"><span class="icon-user-minus"></span> Desintitular ouvidor</a>
											<?php endif ?>
										</div>					
									</div>										
									<div class="row col col-7 forms text-area">
										<div class="col col-12 row grups">
											<p>Matrícula: <strong><?php echo $usuario['matricula']; ?></strong></p>
										</div>
										<div class="col-12 row grups">
											<p>E-mail: <strong><?php echo $usuario['email']; ?></strong></p>
										</div>
										<div class="col-12 row grups center fazul">
											<p>Setor: <strong><?php echo $usuario['setor']; ?></strong></p>
										</div>
										<div class="col-12 row grups justify">
											<p><span class="icon-info"></span> Ao definir este usuário como <strong>ouvidor</strong>, o mesmo será encarregado de <strong>responder os manifestos</strong> direcionados ao setor referido em seu cadastro e terá acesso privilegiado ao <strong>detalhamento</strong> das manifestações. A ação só será devidamente efetuada se o cargo de ouvidor do setor de <strong><?php echo $usuario['setor']; ?></strong> não estiver preenchido ou caso o usuário já for Ouvidor.</p>
										</div>
									</div>
								</div>
							</div>
